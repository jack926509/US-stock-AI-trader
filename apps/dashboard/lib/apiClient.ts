...
export const api = {
  ...
  getSignals: (limit = 100, status?: string) =>
    apiFetch<import("@/types").Signal[]>(
      `/api/signals?limit=${limit}${status ? `&status=${status}` : ""}`
    ),
  ...
  getPnlHistory: (days = 30) =>
    apiFetch<import("@/types").DailyStats[]>(`/api/pnl/history?days=${days}`),
  getAccount: () => apiFetch<import("@/types").Account>("/api/account"),
  getHealth: () =>
    apiFetch<{ status: string; marketOpen: boolean; timestamp: string }>("/api/health"),
  getBacktest: (from?: string, to?: string) =>
    apiFetch<import("@/types").BacktestStats>(
      `/api/backtest${from && to ? `?from=${from}&to=${to}` : ""}`
    ),
