  ...
  return (
    <>
      ...
      >
        ...
        <td className="py-3 pr-3 font-mono text-right text-blue-300">
          {signal.rr_ratio.toFixed(2)}
        </td>
        <td className="py-3 pr-3 text-right">
          {signal.python_confidence !== null ? (
            <span className="text-xs font-mono">
              {(signal.python_confidence * 100).toFixed(0)}%
            </span>
          ) : (
        ...
        <td className="py-3 text-right text-gray-500 text-xs">
          {new Date(signal.received_at).toLocaleString("zh-TW", {
            month: "2-digit",
            day: "2-digit",
            hour: "2-digit",
            minute: "2-digit",
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
              <div>
                ...
                <div className="space-y-1 text-gray-300">
                  <div>
                    進場: <span className="font-mono">${signal.entry_price.toFixed(2)}</span>
                  </div>
                  <div>
                    止損:{" "}
                  ...
                  <div>
                    R:R: <span className="font-mono">{signal.rr_ratio.toFixed(2)}</span>
                  </div>
              ...
              {indicators && (
                <div>
                  ...
                  <div className="space-y-1 text-gray-300">
                    <div>
                      MA 趨勢: <span className="font-mono">{indicators.ma_trend}</span>
                    </div>
                    <div>
                      RSI:{" "}
                      <span className="font-mono">
                        {indicators.rsi_value} ({indicators.rsi_zone})
                      </span>
                    </div>
                    <div>
                      MACD 交叉: <span className="font-mono">{indicators.macd_cross}</span>
                    </div>
                  </div>
                </div>
              )}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={refresh} className="text-blue-400 text-sm hover:text-blue-300">
            刷新
          </button>
        </div>
      </div>
