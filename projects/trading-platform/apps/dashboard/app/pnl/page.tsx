    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          {
            label: "今日已實現損益",
            value: today ? `${today.realized_pnl >= 0 ? "+" : ""}$${fmt(today.realized_pnl)}` : "—",
            color: today ? pnlColor(today.realized_pnl) : "text-gray-300",
          },
          {
            label: "未實現損益",
            value: today ? `${today.unrealized_pnl >= 0 ? "+" : ""}$${fmt(today.unrealized_pnl)}` : "—",
            color: today ? pnlColor(today.unrealized_pnl) : "text-gray-300",
          },
          ...
          <div key={card.label} className="bg-gray-900 rounded-lg p-4">
            ...
            <div className={`text-2xl font-bold font-mono ${card.color}`}>
              {loading && !today ? "..." : String(card.value)}
            </div>
          </div>
        ))}
      </div>
              ...
              {history.map((d) => (
                <tr key={d.date} className="border-b border-gray-800/40 hover:bg-gray-800/30">
                  ...
                  <td className="py-2 text-right">
                    {d.trades_taken > 0
                      ? `${((d.trades_won / d.trades_taken) * 100).toFixed(0)}%`
                      : "—"}
                  </td>
                </tr>
              ))}
