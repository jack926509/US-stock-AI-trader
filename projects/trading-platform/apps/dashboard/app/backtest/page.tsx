...
export default function BacktestPage() {
  ...
  const metrics = [
    ...
    {
      label: "總損益",
      value: stats ? `${stats.totalPnl >= 0 ? "+" : ""}$${fmt(stats.totalPnl)}` : "—",
      color: stats ? pnlColor(stats.totalPnl) : "text-white",
    },
    {
      label: "最大虧損",
      value: stats ? `-$${fmt(stats.maxDrawdown)}` : "—",
      color: "text-red-400",
    },
    {
      label: "最佳交易",
    ...
    {
      label: "最差交易",
      value: stats ? `-$${fmt(Math.abs(stats.worstTrade))}` : "—",
      color: "text-red-400",
    },
  ];
                ...
                {stats.trades.map((t) => (
                  <tr key={t.id} className="border-b border-gray-800/40 hover:bg-gray-800/30">
                    ...
                    <td className="py-2 pr-4 text-right font-mono text-blue-300">
                      {t.rr_achieved.toFixed(2)}
                    </td>
                    <td className="py-2 text-right text-gray-400 text-xs">
                      {new Date(t.closed_at).toLocaleString("zh-TW", {
                        month: "2-digit",
                        day: "2-digit",
                        hour: "2-digit",
                        minute: "2-digit",
