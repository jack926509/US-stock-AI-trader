...
export function SignalTypeBadge({ type }: { type: string }) {
  return (
    <span className={`px-2 py-0.5 rounded text-xs font-bold ${TYPE_COLORS[type] ?? 'bg-gray-800 text-gray-300'}`}>
      {type}
    </span>
  );
}
...
export function StatusBadge({ status }: { status: string }) {
  return (
    <span className={`px-2 py-0.5 rounded text-xs font-semibold ${STATUS_COLORS[status] ?? 'bg-gray-800 text-gray-400'}`}>
