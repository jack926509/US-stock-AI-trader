...
export function PositionTable({ positions, onClose }: Props) {
  ...
  if (positions.length === 0) {
    return (
      <div className="text-center py-16 text-gray-500">目前無持倉</div>
    );
  }
          ...
          {positions.map((p) => (
            <tr key={p.symbol} className="border-b border-gray-800/50 hover:bg-gray-900/40">
              ...
              <td className="py-3 pr-4">
                <span className={`px-2 py-0.5 rounded text-xs font-semibold ${p.side === 'long' ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300'}`}>
